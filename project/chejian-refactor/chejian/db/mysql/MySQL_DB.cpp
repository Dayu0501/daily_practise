...
bool MySQL_DB::initialize_MySQL_library()
{
    ...
    if (init_flag) {
        printf("initialize_MySQL_library() has been called successfully, do not need to be call again! \n");
        return false;
    }
...
bool MySQL_DB::connect(const char *host, unsigned int port, const char *user, const char *passwd, const char *db, bool statrments)
{

	if (host == NULL) {
        //printf("Try to establish connection with localhost:%d@%s DataBase: %s \n", port, user, db);
	} else {
        //printf("Try to establish connection with %s:%d@%s DataBase: %s \n", host, port, user, db);
	}
    ...
    */
    ...
    if (mysql_real_connect(&mysql, host, user, passwd, db, port, NULL, floatClient) == NULL)
    {
...
bool MySQL_DB::noReturnWork(const char *sql)
{
    if (mysql_query(&mysql, sql) != 0) {
		printf("%s \n",sql);
        LOG(DB,ERR,"NoReturnDbWork failed! Reason: %s \n",mysql_error(&mysql));
        printf("noReturnWork()  \n");
        printf("Reason: %s \n", mysql_error(&mysql));
        return false;
    }
