...
bool _c_algClass_0159_HuaiBei::ResultEdit()
{
    LOG(ALG, INFO, "_c_algClass_0159_HuaiBei::ResultEdit() begin");
    ...
    {
        LOG(ALG, INFO, "_c_algClass_0159_HuaiBei::ResultEdit() result[%d]: %d", i, vec_luosi[i]->value->result);
    ...
    {   //需2颗螺丝
        LOG(ALG, INFO, "_c_algClass_0159_HuaiBei::ResultEdit() < begin_date");
        if (luosi_count >= 2)
    ...
    LOG(ALG, INFO, "_c_algClass_0159_HuaiBei::ResultEdit() chuCiDengJiRiQi: %s, begin_date: %s", chuCiDengJiRiQi.c_str(), begin_date.c_str());
    LOG(ALG, INFO, "_c_algClass_0159_HuaiBei::ResultEdit() luosi_count: %d, result: %d", luosi_count, luoSi->value->result);
    return true;
}
