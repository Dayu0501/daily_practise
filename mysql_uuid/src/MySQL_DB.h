...
public:
    ...
    bool connect(const char *host, unsigned int port, const char *user,
                 ...
                 bool statrments); /* host可以为NULL，表示本机；其他参数不能为NULL */
    void disconnect();

    bool createTable(const char *name,
                     const char *format); /* 创建Table，如果已存在则不会创建 */
    bool dropTable(const char *name);

    bool insert(const char *tableName, const char *valueFormat);

    bool select(const char *tableName,
                const char *format); /* 将SELECT结果输出到终端(需要根据实际需求修改) */
