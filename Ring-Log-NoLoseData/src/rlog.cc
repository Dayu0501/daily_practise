...
cell_buffer* create_cell_buffer(key_t shm_key, uint32_t total_len) {
  ...
  int shmid = shmget(shm_key, sizeof(cell_buffer) + total_len,
...
/*
该函数查找对应改路径的的key，清除之前的内核中的共享内存的对象。
*/
int get_main_shm() {
  char exec_path[4096];
...
ring_log::ring_log()
    : _buff_cnt(3),
      ...
      _lst_lts(0),
      _tm() {
  //初始化的时候创建了两个共享内存的对象，也就是两个cell
  // buffer，其中一个是head。
  if (_buff_cnt < 2) _buff_cnt = 2;
...
void ring_log::init_path(const char* log_dir, const char* prog_name,
                         int level) {
...
void ring_log::try_append(const char* lvl, const char* format, ...) {
  ...
  // int prev_len = snprintf(log_line, LOG_LEN_LIMIT, "%s[%d-%02d-%02d
  // %02d:%02d:%02d.%03d]", lvl, _tm.year, _tm.mon, _tm.day, _tm.hour, _tm.min,
  // _tm.sec, ms);
  int prev_len =
      snprintf(log_line, LOG_LEN_LIMIT, "%s[%s.%03d]", lvl, _tm.utc_fmt, ms);
      ...
      if (next_buf->status == cell_buffer::FULL) {
        ...
        if (_one_buff_len * (_buff_cnt + 1) > MEM_USE_LIMIT) {
          ...
          cell_buffer* new_buffer =
              create_cell_buffer(SHM_KEY_ID_SEQ + _buff_cnt, _one_buff_len);
          _buff_cnt += 1;
...
bool ring_log::decis_file(int year, int mon, int day) {
  ...
  if (!_fp) {
    ...
    char log_path[1024] = {};
    sprintf(log_path, "%s/%s.%d%02d%02d.%u.log", _log_dir, _prog_name, _year,
            _mon, _day, _pid);
    ...
    for (int i = _log_cnt - 1; i > 0; --i) {
      sprintf(old_path, "%s/%s.%d%02d%02d.%u.log.%d", _log_dir, _prog_name,
              _year, _mon, _day, _pid, i);
      sprintf(new_path, "%s/%s.%d%02d%02d.%u.log.%d", _log_dir, _prog_name,
              _year, _mon, _day, _pid, i + 1);
      rename(old_path, new_path);
    }
    // mv xxx.log xxx.log.1
    sprintf(old_path, "%s/%s.%d%02d%02d.%u.log", _log_dir, _prog_name, _year,
            _mon, _day, _pid);
    sprintf(new_path, "%s/%s.%d%02d%02d.%u.log.1", _log_dir, _prog_name, _year,
            _mon, _day, _pid);
    rename(old_path, new_path);
